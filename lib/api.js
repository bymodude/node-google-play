...
const GooglePlay = function GooglePlay (username, password, androidId, useCache, _debug, requestsDefaultParams) {
   ...
   */
  function _executeRequestApi2 (path, query, datapost, contentType, country, language) {
   ...
   */
  function executeRequestApi (path, query, datapost, contentType, country, language) {
    return memoizedExecuteRequestApi2(path, query, datapost, contentType, country, language)
    .then(function (body) {
  ...
  /**
   * Get a package's full (i.e. including badge image urls) current details.
   */
  function getPackageDetailsFull(pkg, country, language) {
    return executeRequestApi('details', {doc: pkg}, null, null, country, language).then(function (res) {
  ...
  return {
    ...
    detailsFull : function detailsFull(pkg, country, language, cb) {
      return getPackageDetailsFull(pkg, country, language).nodeify(cb);
    },
